    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Comparing <span className="font-medium">{columnCount}</span> motorcycles
          {identicalCount > 0 && ` · ${identicalCount} identical specifications`}
        </p>
      ...
      <div className="overflow-x-auto rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <table className="w-full min-w-3xl border-collapse text-left">
          <caption className="sr-only">
            Side-by-side specification comparison of {columnCount} motorcycles
          </caption>
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {onRemove && (
                        <button
                          ...
                          aria-label={`Remove ${formatDisplayName(motorcycle)} from comparison`}
      ...
      {differencesOnly && visibleGroups.length === 0 && (
        <p className="mt-6 text-center text-sm text-zinc-500 dark:text-zinc-400">
          These motorcycles have identical published specifications.
        </p>
      )}
    </div>
  );
